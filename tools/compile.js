...
let bossDanmakuJs = '';
if (fs.existsSync(path.join(jsDir, 'bossdanmakudata.js'))) {
    bossDanmakuJs = fs.readFileSync(path.join(jsDir, 'bossdanmakudata.js'), 'utf8');
}
...
window.compiledDanmaku = window.compiledDanmaku || {};
window.compiledBossDanmakuList = ${JSON.stringify(window.bossDanmakuList, null, 2)};
window.bossDanmakuList = window.compiledBossDanmakuList;

`;
            let bossCompiledCount = 0;
            for (let spell of window.bossDanmakuList) {
                let spellId = spell.id;
                if (!spellId) continue;

                // Compile emitterScript
                let blocks = Array.isArray(spell.emitterScript) ? spell.emitterScript : (typeof codeToBlocks === 'function' ? codeToBlocks(spell.emitterScript) : []);
                let compiledBlocks = typeof compileIndentedBlocks === 'function' ? compileIndentedBlocks(JSON.parse(JSON.stringify(blocks))) : [];
                let threadGroups = typeof window.DanmakuCompiler.splitParallelThreadGroups === 'function'
                    ? window.DanmakuCompiler.splitParallelThreadGroups(compiledBlocks)
                    : null;
                ...
                if (threadGroups && threadGroups.length >= 2) {
                    let funcs = threadGroups.map(group => window.DanmakuCompiler.compileSingle(group));
                    bossOutputJS += `window.compiledBossDanmaku['${spellId}'] = window.compiledDanmaku['${spellId}'] = [\n  ${funcs.join(',\n  ')}\n];\n`;
                } else {
                    let funcStr = window.DanmakuCompiler.compileSingle(compiledBlocks);
                    bossOutputJS += `window.compiledBossDanmaku['${spellId}'] = window.compiledDanmaku['${spellId}'] = ${funcStr};\n`;
                }

                // Compile bulletScript
                let bBlocks = Array.isArray(spell.bulletScript) ? spell.bulletScript : (typeof codeToBlocks === 'function' ? codeToBlocks(spell.bulletScript) : []);
                let bCompiled = typeof compileIndentedBlocks === 'function' ? compileIndentedBlocks(JSON.parse(JSON.stringify(bBlocks))) : [];
                let bFuncStr = window.DanmakuCompiler.compileSingle(bCompiled, true);
                bossOutputJS += `window.compiledBossDanmaku['${spellId}_bullet'] = window.compiledDanmaku['${spellId}_bullet'] = ${bFuncStr};\n`;

                // Compile magicCircleScript
                let mBlocks = Array.isArray(spell.magicCircleScript) ? spell.magicCircleScript : (typeof codeToBlocks === 'function' ? codeToBlocks(spell.magicCircleScript) : []);
                let mCompiled = typeof compileIndentedBlocks === 'function' ? compileIndentedBlocks(JSON.parse(JSON.stringify(mBlocks))) : [];
                let mFuncStr = window.DanmakuCompiler.compileSingle(mCompiled, true);
                bossOutputJS += `window.compiledBossDanmaku['${spellId}_magic'] = window.compiledDanmaku['${spellId}_magic'] = ${mFuncStr};\n\n`;

                bossCompiledCount++;
            }
            fs.writeFileSync(path.join(jsDir, 'bossdanmakucompiledata.js'), bossOutputJS, 'utf8');
            console.log(`Successfully compiled ${bossCompiledCount} boss spell scripts to js/bossdanmakucompiledata.js!`);
        }
